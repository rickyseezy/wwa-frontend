...
const InputNtexterea = ({titleInput, placeholder, bolea} : ControlInputNtextera) => {
    ...
    if (!bolea) {
          if(tabfile.length === 0){
            ...
            <img className={styles["files__icon"]} src="/icons/socials-yt.png"/> < div className = {
       styles["files__text"]
            } > Importez des photos et des vidéos depuis votre appareil </div>
      ...
      return (
        <>
        <div>
        <h4 className={styles['add-text']} >5. Ajoutez des images et vidéos à votre projet</h4>
          ...
          <div className={styles['step-grid']}>
             ...
             }
             {
                tabfile.length > 0 ? <>
                   ...
                   <InputFile  select={INputValue} classtyle={'input-file__input'}  />
                   </div>
                </> : null
             }
          </div >
